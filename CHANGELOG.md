...
- **Bug no indexExports**: Corrigido erro `config.indexExports?.includes is not a function` quando `indexExports` é `true` (boolean) ao invés de array
- Agora aceita `indexExports` como boolean (`true`/`false`) ou array de tipos específicos
